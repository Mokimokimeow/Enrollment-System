...
async function handleLogin(e) {
    ...
    if (role === 'admin') {
        try {
            ...
            console.warn('DB admin login failed, trying fallback:', error);
        }
        ...
        if (email === 'admin@biringan.edu' && password === 'admin123') {
            ...
            showToast('Invalid admin credentials. Use admin@university.local / password123 or admin@biringan.edu / admin123', 'error');
            return;
        }
    }

    // Demo student login (temporary accounts)
    if (role === 'student') {
        ...
        const student = demoStudents.find(s => s.email === email && s.password === password);
        
        if (student) {
            console.log('Student found:', student);
            currentUser = { email, role: 'student', name: student.name, level: student.level, strand: student.strand };
            console.log('Current user set:', currentUser);
            hideLoginModal();
            console.log('Login modal hidden, redirecting to student dashboard');
...
function populateStepSummary(stepNum) {
    ...
    if (stepNum === 1) {
        ...
        const fields = [
            ...
            { label: 'Region', val: document.getElementById('modalRegion').options[document.getElementById('modalRegion').selectedIndex]?.text || '—' },
            { label: 'Province', val: document.getElementById('modalProvince').options[document.getElementById('modalProvince').selectedIndex]?.text || '—' },
            { label: 'Town/City', val: document.getElementById('modalCity').options[document.getElementById('modalCity').selectedIndex]?.text || '—' },
            { label: 'Barangay', val: document.getElementById('modalBarangay').options[document.getElementById('modalBarangay').selectedIndex]?.text || '—' },
        ...
        const fields = [
            ...
            { label: 'Elem Year Graduated', val: document.getElementById('modalElementaryYearGraduated').value },
